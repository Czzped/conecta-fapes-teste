# SPRINT-{NNN}

[← Voltar ao Management](../README.md)

| Atributo | Valor |
|----------|-------|
| **Periodo** | YYYY-MM-DD a YYYY-MM-DD |
| **Milestone** | `MS-XX — Nome do Milestone` |
| **GitHub Milestone** | `milestone: MS-XX` |
| **Goal** | Uma frase descrevendo o objetivo principal do sprint |

---

## Epics Comprometidos

> Cada epic e uma issue no GitHub com label `epic`, atribuida ao milestone do sprint.
> Cada US e uma issue com label `user-story`, filha do epic. Tasks tem label `task`.

### Domain {NN} — {Nome do Domain}

| Epic | Modulo | GitHub Issue | Status |
|------|--------|-------------|--------|
| `EPIC-MXXX-001` | MXXX | #000 | To Do / In Progress / Done |

### Domain {NN} — {Nome do Domain}

| Epic | Modulo | GitHub Issue | Status |
|------|--------|-------------|--------|
| `EPIC-MXXX-001` | MXXX | #000 | To Do / In Progress / Done |

---

## Review

### Entregue

- ...

### Nao entregue

- ... (motivo)

---

## Retrospectiva

### O que funcionou

- ...

### O que melhorar

- ...

### Acoes para o proximo sprint

- ...
