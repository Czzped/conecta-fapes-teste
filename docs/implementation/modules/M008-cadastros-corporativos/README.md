...
Neste contexto, a propria agencia de fomento e representada como uma instancia de `Instituicao`. Toda `Instituicao` e composta por `UnidadeOrganizacional`, permitindo descrever tanto organizacoes externas quanto a estrutura interna da agencia de forma uniforme. As `AreaTecnica` da agencia sao um subtipo de `UnidadeOrganizacional`, preservando o termo amplamente usado nos modulos consumidores sem criar uma estrutura paralela ao organograma institucional.
...
| RN03 | Unidades organizacionais possuem hierarquia pai-filho e pertencem a exatamente uma instituicao. | Must |
...
| RN08 | Um servidor da agencia deve estar vinculado a exatamente uma AreaTecnica, e toda AreaTecnica e uma UnidadeOrganizacional da instituicao agencia. | Must |
