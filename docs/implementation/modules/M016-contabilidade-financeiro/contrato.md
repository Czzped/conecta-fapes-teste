...
| M010 | Referencia `ContaBancaria` como destino de aporte financeiro em `RegistrarAporteFinanceiro` (campo `contaBancariaDestinoId`) e emite o evento `TaxaGestaoParceriasCalculada` consumido pelo subdominio taxa-gestao |
...
| CadastrarPlanoAplicacaoAcaoTransversal | Command | Planejar uso da Taxa de Gestao de Parcerias custodiada por rubricas permitidas | outorgaId, acaoTransversalId, itens[{rubricaId, valorPrevisto, justificativa}] | `PlanoAplicacaoAcaoTransversal` criado | RN12, RN15 | Outorga existente, com `TaxaGestaoParcerias` (taxa-gestao) classificada e saldo disponivel | Plano excede valor vinculado, rubrica invalida, outorga sem classificacao contabil | Nao | Gestor financeiro | API interna/backoffice a definir |
| RegistrarDespesaAcaoTransversal | Command | Registrar despesa institucional vinculada a outorga/acao transversal e a item/rubrica planejada | outorgaId, acaoTransversalId, itemPlanoAplicacaoId?, rubricaId, valor, data, documentoId, justificativa | `DespesaAcaoTransversal` registrada | RN13, RN14 | Outorga e rubrica validas; documento informado; item do plano informado quando houver plano aprovado | Rubrica nao permitida, saldo insuficiente, documento ausente, item do plano invalido | Nao | Gestor financeiro | API interna/backoffice a definir |
| AnalisarPrestacaoContasAcaoTransversal | Command | Aprovar, glosar, solicitar ajuste ou reprovar despesas da Acao Transversal | prestacaoId, decisao, itensGlosados?, parecer | `PrestacaoContasAcaoTransversal` atualizada | RN13, RN14 | Prestacao em analise | Estado invalido, parecer ausente | Nao | Analista financeiro | API interna/backoffice a definir |
| ConsultarDashboardAcaoTransversal | Query | Consultar valor custodiado, planejado, executado, aprovado, glosado e saldo por outorga, acao transversal, rubrica e periodo | filtros | Consolidado de Acao Transversal | RN11, RN12, RN13 | — | Filtro invalido | N/A | Gestor financeiro, Diretoria, M018, M019 | API interna/backoffice a definir |

> **Recebimento e classificacao da Taxa de Gestao de Parcerias nao pertencem mais a este contrato.** A parametrizacao da politica/faixas, o recebimento do calculo do M010 (evento `TaxaGestaoParceriasCalculada`) e a classificacao/repasse da taxa sao de responsabilidade do subdominio [taxa-gestao](taxa-gestao/README.md). As operacoes acima de Acao Transversal modelam **somente a execucao** dos recursos custodiados e referenciam a entidade `TaxaGestaoParcerias` (taxa-gestao) por meio de `OutorgaAcaoTransversal` (`outorgaId` / `acaoTransversalId`).
