# M003 - Gestao de Projetos Captados
...
| [Modelo Estrutural](modelo-estrutural.md) | Entidades de projeto, plano, resultados, equipe, orcamento e execucao consolidada |
| [Proposta: Ciclo de Fomento da Projeto](specifications/proposta-ciclo-fomento-projeto.md) | Proposta de timeline pre-award, award e post-award como read model transversal |
| [Diarias da Projeto](diarias/README.md) | Subfluxo dedicado para solicitacao, aceite, alocacao, remocao antes do inicio e regularizacao de diarias; abrangencia, tipo de diaria e parametros de calculo sao referencias do M008 |
| [Aditivos da Projeto](aditivos/README.md) | Subfluxo dedicado para vigencia, orcamento original e dados dos aditivos da projeto |
...
Apos a contratacao, a agencia de fomento precisa acompanhar a projeto apoiada como um objeto unico: seus dados estaveis, ortogado, plano vigente, objetivos, resultados, riscos, beneficios, equipe planejada, cronograma, orcamento planejado, execucao consolidada e alteracoes autorizadas de rubrica.

O M003 concentra a gestao pos-contratacao da `Projeto`. Ele nao e dono de edital, cotas de edital, alocacao de bolsistas ou prestacao de contas detalhada. Esses conceitos pertencem a outros modulos e sao consumidos pelo M003 apenas como referencias ou integracoes.

> **Fronteira com M011:** `Edital`, chamada, inscricao, avaliacao, recurso, resultado e configuracao da captacao sao responsabilidade do M011. O M003 assume a projeto depois da contratacao/outorga.
...
No ConectaFAPES, `Projeto` e o conceito operacional unico para o item apoiado pela agencia apos a contratacao. Uma projeto pode representar projeto de pesquisa, projeto de inovacao, visita tecnica ou outro tipo cadastrado em `TipoProjeto`.

A `Projeto` possui um `Ortogado`, que e o papel assumido por uma `PessoaFisica` no contexto da outorga. O ortogado ativo pode solicitar inclusao ou retirada de rubricas do orcamento planejado.

Os elementos planejaveis da projeto ficam em `VersaoPlanoProjeto`. Resultados, objetivos, riscos, beneficios, equipe, cronograma e orcamento planejado pertencem a uma versao do plano. Alteracoes relevantes geram nova versao, preservando historico.

O `OrcamentoPlanejado` registra a previsao aprovada de recursos para executar a projeto. O `OrcamentoExecutado` e uma visao consolidada e historica da execucao, calculada a partir de `LancamentoExecucao` e integrada aos modulos financeiros quando necessario.
...
A consulta de vigencia e aditivos preserva a data de aprovacao original, a data inicial, a data final original, a data final vigente e o orcamento original da projeto. A data final vigente so muda por aditivo de tempo aprovado; o orcamento original permanece historico mesmo quando houver aditivo financeiro. A tela **Meu Projeto** deve exibir o bloco **Vigencia e aditivos** com abas **Resumo** e **Dados dos aditivos**; a area **Projetos** pode expor uma aba **Aditivos** para consulta dos mesmos registros.
...
|----|-----------|------------|
| RN01 | `Projeto` e a entidade operacional unica para o item contratado no modelo do M003. | Must |
| RN02 | A projeto pode possuir `TipoProjeto` quando a configuracao da captacao exigir essa classificacao. | Must |
| RN03 | Toda projeto deve possuir um `Ortogado` ativo vinculado a uma `PessoaFisica` do M008. | Must |
| RN04 | A projeto pode possuir versoes de plano quando a captacao exigir planejamento; apenas uma versao pode estar `VIGENTE`. | Must |
...
| RN10 | O orcamento executado da projeto deve ser calculado a partir de lancamentos de execucao, preservando historico de movimentos. | Must |
...
| RN16 | A projeto pode possuir uma timeline transversal em `EstagioCicloFomento`, com marcos de pre-award, award e post-award. | Must |
...
| RN19 | Apenas um estagio da timeline pode estar `ATUAL` por projeto. | Must |
| RN20 | `CONCLUIDO` e `CANCELADA` sao marcos terminais alternativos do ciclo; a projeto nao pode finalizar simultaneamente nos dois. | Must |
...
| RN22 | O M003 e dono da solicitacao operacional de diaria da projeto, incluindo alocacao do bolsista, periodo de deslocamento, calculo automatico e aceite. | Must |
| RN23 | A solicitacao de diaria deve estar vinculada a uma projeto ativa e a uma `AlocacaoBolsista` valida consultada em M009. | Must |
...
| RN31 | O debito gerado pelo comprometimento da diaria deve ser rastreavel ate a `SolicitacaoDiaria` e compor a execucao consolidada da projeto. | Must |
...
| RN34 | Toda projeto deve preservar data de aprovacao original, data inicial, data final original, data final vigente e orcamento original para consulta historica. | Must |
...
| RN38 | A existencia de aditivo de tempo nao altera por si so o orcamento original da projeto. | Must |
...
| RN42 | A visao orcamentaria por rubrica em **Meu Projeto** deve ser exibida somente para coordenador da projeto, contendo total, consumido, alocado quando aplicavel, disponivel e percentuais por rubrica. Saldos seguem definicao canonica de [RN-SLD01](../../../discovery/regras-saldo-alocado-disponivel.md). | Must |
