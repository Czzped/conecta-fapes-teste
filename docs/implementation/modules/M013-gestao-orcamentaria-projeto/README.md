...
O orcamento do projeto nao e uma conta contabil. O M013 gerencia o planejamento/limite aprovado por rubricas da Iniciativa/Projeto e as transacoes que movimentam essas rubricas. O M016 gerencia contas contabeis, fundos financeiros, contas bancarias, centros de custo, lancamentos contabeis, saldos financeiros e conciliacao.

| Conceito | Dono | Exemplo |
|----------|------|---------|
| Orcamento do projeto | M013 | Projeto com R$ 100.000,00 aprovados. |
| Rubrica do projeto | M013/M008 | Bolsas, diarias, material de consumo. |
| Transacao da rubrica | M013 | Comprometimento, execucao, estorno ou remanejamento que altera saldo de uma rubrica. |
| Movimento bancario | M014/M016 | Movimento de conta/extrato conciliado com a prestacao ou conta bancaria. |
...
O saldo de cada rubrica deve ser atualizado em tempo real, refletindo comprometimentos, execucoes, estornos e movimentacoes aprovadas. Esses fatos devem ser registrados como `Transacao`, sempre referenciando uma `RubricaProjeto`. A `RubricaProjeto` e a categoria/limite aprovado; a `Transacao` e o movimento da categoria. Movimentos bancarios pertencem a M014/M016 e podem ser conciliados por referencia quando necessario.

As rubricas disponiveis para um projeto devem estar previamente cadastradas no modulo de cadastros corporativos (M008).

> Projetos e editais sao gerenciados por M003. Pagamentos sao gerenciados por M004. Rubricas corporativas sao gerenciadas por M008 e especializadas neste modulo como `RubricaProjeto`. Este modulo consome essas informacoes para operacionalizar a gestao orcamentaria.
