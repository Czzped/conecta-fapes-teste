...
O M021 centraliza a base de formularios reutilizaveis da FAPES. Funcionarios da FAPES podem criar formularios e categorias de formulario, e qualquer usuario logado pode criar respostas a formularios, enquanto a edicao de formularios e respostas fica restrita ao autor do registro e a publicacao, reversao, inativacao e exclusao de formularios seguem as permissoes especificas do modulo. A ideia é que as funções deste módulo sejam utilizadas para criar e gerenciar formulários a serem usados com diversas finalidades em outros módulos do ConectaFapes. O primeiro uso será no módulo M011 para submissao de propostas de projetos, avaliação de habilitação e avaliacao ad hoc.
...
Quanto ao permissionamento, qualquer funcionario da FAPES autenticado podera criar um formulario ou gerenciar categorias de formulario, enquanto qualquer usuario logado podera criar uma resposta a formulario. A edicao de um formulario ou de uma resposta, contudo, sera restrita ao proprio usuario autor do registro, respeitando tambem as restricoes de estado aplicaveis. A criacao de um formulario a partir de outro formulario existente nao depende da autoria do formulario original: qualquer funcionario da FAPES autenticado podera criar sua propria copia a partir de qualquer formulario disponivel.
---
...
| RN28 | Qualquer funcionario da FAPES autenticado pode criar um formulario e qualquer usuario logado pode criar uma resposta a formulario. |
| RN29 | Um formulario ou uma resposta a formulario so pode ser editado pelo proprio usuario autor do registro, respeitadas as restricoes de estado aplicaveis. |
| RN30 | Qualquer funcionario da FAPES autenticado pode criar um formulario a partir de qualquer formulario existente, independentemente da autoria do formulario original. |
| RN31 | Qualquer funcionario da FAPES autenticado pode criar, consultar, atualizar, excluir e listar categorias de formulario, respeitadas as demais regras aplicaveis. |
